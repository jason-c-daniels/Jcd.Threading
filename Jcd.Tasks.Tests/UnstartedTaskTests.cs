...
public class UnstartedTaskTests
{
    ...
    public async Task Create_Called_With_Action_And_Options_Creates_A_Task_Which_Executes_The_Provided_Action_When_Start_Is_Called(TaskCreationOptions options)
    {
        var actionWasCalled = false;
        void LocalAction() { actionWasCalled = true;}
    ...
    public async Task Create_Called_With_Func_And_Options_Creates_A_Task_Which_Executes_The_Provided_Action_When_Start_Is_Called(TaskCreationOptions options, bool funcInput)
    {
    ...
    [InlineData(TaskCreationOptions.AttachedToParent)]
    [InlineData(TaskCreationOptions.RunContinuationsAsynchronously)]
    public async Task Create_Called_With_Async_Action_And_Options_Creates_A_Task_Which_Executes_The_Provided_Action_When_Start_Is_Called(TaskCreationOptions options)
    {
        var actionWasCalled = false;

        Task LocalAsyncAction() { actionWasCalled = true; return Task.CompletedTask; }
    ...
    [InlineData(TaskCreationOptions.AttachedToParent,false)]
    [InlineData(TaskCreationOptions.RunContinuationsAsynchronously,true)]
    public async Task Create_Called_With_Async_Func_And_Options_Creates_A_Task_Which_Executes_The_Provided_Action_When_Start_Is_Called(TaskCreationOptions options, bool funcInput)
    {
    ...
    [Fact]
    public void Create_Called_With_An_Async_Action_And_CancellationToken_Which_Gets_Canceled_Task_Then_Has_Status_Of_Canceled()
    {
        Task LocalAction() { 
    ...
    [Fact]
    public void Create_Called_With_An_Async_Func_And_CancellationToken_Which_Gets_Canceled_Task_Then_Has_Status_Of_Canceled()
    {
        Task<bool> Invert(bool x) => Task.FromResult(!x);
        var cts = new CancellationTokenSource();
        var task = UnstartedTask.Create(()=>Invert(true), cts.Token);
