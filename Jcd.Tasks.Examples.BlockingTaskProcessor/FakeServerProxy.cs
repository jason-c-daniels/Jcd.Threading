...
public class FakeServerProxy
{
   ...
   public async Task SendRequest(int bufferType, byte[] buffer)
   {
      if (_logDetailedCallInformation)
         Console.WriteLine($"{DateTime.Now:yyyy-MM-dd hh:mm:ss.FFFF} {nameof(FakeServerProxy)}{nameof(SendRequest)} started. {BacklogCounter.Value} calls from other tasks are already waiting."
                          );
      ...
      using (await _lock.LockAsync(_cts.Token))
      {
         ...
         if (_logDetailedCallInformation)
            Console.WriteLine($"{DateTime.Now:yyyy-MM-dd hh:mm:ss.FFFF} {nameof(FakeServerProxy)}{nameof(SendRequest)} lock acquired with {backlog} other tasks waiting for this call to complete."
                             );
