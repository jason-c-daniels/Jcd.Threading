   ...
   where T : ProcessExecutionBase<T>, new()
{
   ...
   )
   {
      ...
      var makeCallsTask =
         CreateMakeLotsOfCallsTask(taskSchedulingFrequencyInMs, lifespanInSeconds, maxTasks, logRequestScheduling);
   ...
   private Task CreateMakeLotsOfCallsTask(
      double scheduleDelayInMs        = 100
      ...
      return Task.Run(async () =>
                      {
                         ...
                         {
                            await WaitToScheduleLotsOfCalls(scheduleDelayInMs
                                                          , numberOfMakeRequestTasks
                                                          , logSchedulingRequests
                                                          , cts
                                                           );
   ...
   private async Task WaitToScheduleLotsOfCalls(
   ...
   private void ScheduleLotsOfCalls(int numberOfMakeRequestTasks, Random rnd, CancellationTokenSource cts)
   {
      ...
      for (var i = 0; i < numberOfMakeRequestTasks; i++) ScheduleASingleCall(rnd, cts, i);
   }

   private Task CreatePingTask(
      double scheduleDelayInMs    = 1000
    , double lifeSpanInSeconds    = 120
    , bool   logRequestScheduling = true
   )
   {
   ...
   protected async Task ExecutePing(
   ...
   private async Task ReportPingResults(DateTime finishedAt, DateTime startedAt, DateTime scheduledAt, int backlog)
   {
      ...
      Console.WriteLine($"{DateTime.Now:O} - {MyType.Name}: Ping completed {sinceScheduled.TotalMilliseconds:n2}ms after scheduling, and {sinceStarted.TotalMilliseconds:n2}ms after starting. Concurrent ping backlog {backlog}"
                       );
      await Console.Out.FlushAsync();
      await Task.Yield();
   }
   ...
   protected virtual void LogPingScheduled(bool logRequestScheduling, SynchronizedValue<int> pingBacklog)
   {
      ...
      Console.WriteLine($"{DateTime.Now:O} - {MyType.Name}: Starting to ping... concurrent ping backlog {pingBacklog.Value}"
                       );
      Console.Out.Flush();
   }

   protected void LogSchedulingLotsOfTasks(
      int    numberOfMakeRequestTasks
    , bool   logSchedulingRequests
    , double scheduleDelayInMs
   )
   ...
   protected virtual void SchedulePing(
      SynchronizedValue<int>  pingBacklog
    , Random                  rnd
    , DateTime                scheduledAt
    , CancellationTokenSource cts
